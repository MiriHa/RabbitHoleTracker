...
class CallSensor : AbstractSensor(
    ...
    class CallReceiver : BroadcastReceiver() {
                ...
                if (state == TelephonyManager.EXTRA_STATE_RINGING) {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(object : CallRunnable(cachedCallEvent) {
                        override fun run() {
                            ...
                            val ringingLength = (totalLength - lastCallLogDuration)
                            ...
                            } else if (cachedEvent != null && cachedEvent == ONHOLD) {
                            ...
                            if ("OUTGOING" != lastCallLogType || lastCallLogDuration > 0) { // do not log OUTGOING with duration 0 (#301)
                                ...
                                saveEntry(
                                    timestamp, lastCallLogDuration, lastCallLogType
                                )
                            }
        ...
        private fun saveEntry(
            timestamp: Long, duration: Long, event: String?
        ) {
            LogEvent(
